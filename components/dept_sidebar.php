...
<?php $isSupplyOffice = isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'supply_office'; ?>
<?php 
...
?>
<!-- Standard Department/Office Sidebar -->
<nav class="mt-6">
    <a href="dept_dashboard.php" class="flex items-center px-6 py-3 text-gray-700 hover:bg-gray-50 hover:text-maroon <?php echo (basename($_SERVER['PHP_SELF']) === 'dept_dashboard.php') ? 'text-maroon bg-red-50 border-r-4 border-maroon' : ''; ?>">
    ...
    <div class="relative" id="utilizationDropdown">
        <button type="button" class="flex items-center px-6 py-3 text-gray-700 hover:bg-gray-50 hover:text-maroon w-full justify-between <?php echo in_array(basename($_SERVER['PHP_SELF']), ['utilization__view.php', 'utilization_view_admin.php', 'lib.php', 'ppmp.php']) ? 'text-maroon bg-red-50 border-r-4 border-maroon' : ''; ?>" data-dropdown-toggle="utilizationMenu">
            ...
            <svg class="w-4 h-4 text-gray-500 sidebar-dropdown-arrow transition-transform duration-200 <?php echo in_array(basename($_SERVER['PHP_SELF']), ['utilization__view.php', 'utilization_view_admin.php', 'lib.php']) ? 'rotate-180' : ''; ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
        </button>
        <?php $utilizationMenuOpen = in_array(basename($_SERVER['PHP_SELF']), ['utilization__view.php', 'utilization_view_admin.php', 'lib.php', 'ppmp.php']); ?>
        <div id="utilizationMenu" class="ml-6 mt-1 space-y-1 <?php echo $utilizationMenuOpen ? '' : 'hidden'; ?>">
            ...
            <a href="utilization_view_admin.php" class="flex items-center px-6 py-2 text-sm <?php echo basename($_SERVER['PHP_SELF']) === 'utilization_view_admin.php' ? 'text-maroon bg-red-50 border-r-4 border-maroon font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-maroon'; ?>">
    ...
    <?php if ($isSupplyOffice): ?>
    <a href="purchase_orders.php" class="flex items-center px-6 py-3 text-gray-700 hover:bg-gray-50 hover:text-maroon <?php echo ($currentPage === 'purchase_orders.php') ? 'text-maroon bg-red-50 border-r-4 border-maroon' : ''; ?>">
        <svg class="w-5 h-5 sidebar-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h10v10H7zM7 17h10m-10-6h10M9 5V4a1 1 0 011-1h4a1 1 0 011 1v1"></path>
        </svg>
        <span class="sidebar-text ml-3">Purchase Orders</span>
    </a>
    <?php endif; ?>
    ...
    <a href="notifications.php" class="flex items-center px-6 py-3 text-gray-700 hover:bg-gray-50 hover:text-maroon <?php echo (basename($_SERVER['PHP_SELF']) === 'notifications.php') ? 'text-maroon bg-red-50 border-r-4 border-maroon' : ''; ?>">
        <svg class="w-5 h-5 sidebar-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
