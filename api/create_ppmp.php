...
try {
    ...
    if (empty($fiscalYear)) {
        echo json_encode(['success' => false, 'message' => 'Fiscal Year is required']);
        exit;
    }
    
    // Auto-generate PPMP number based on department and fiscal year
    ...
    // Get the count of existing PPMPs for this department and year
    $countQuery = "SELECT COUNT(*) FROM ppmp WHERE department_id = ? AND fiscal_year = ?";
    ...
    if (empty($fiscalYear) || empty($ppmpNumber)) {
        echo json_encode(['success' => false, 'message' => 'Fiscal Year and PPMP Number are required']);
        exit;
    }
    
    // Allow empty items for draft PPMPs
    if (empty($generalDescriptions) && $isFinal) {
        echo json_encode(['success' => false, 'message' => 'Cannot finalize PPMP without items. Please add at least one item or save as draft.']);
        exit;
    }
    ...
    // Insert PPMP record
    $sql = "INSERT INTO ppmp (department_id, fiscal_year, ppmp_number, ppmp_type, is_indicative, is_final, status, created_by) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($sql);
    $stmt->execute([$departmentId, $fiscalYear, $ppmpNumber, $ppmpType, $isIndicative ? 1 : 0, $isFinal ? 1 : 0, $status, $userId]);
    $ppmpId = $db->lastInsertId();
    
    // Insert PPMP items (only if there are items to insert)
    ...
    if (!empty($generalDescriptions)) {
        // Check if any items have LIB mappings
        ...
        if ($hasLibMappings) {
            try {
                ...
                if (!$syncResult['success']) {
                    error_log("Warning: PPMP created but LIB sync failed: " . ($syncResult['message'] ?? 'Unknown error'));
                }
            } catch (Exception $syncError) {
                error_log("Warning: PPMP created but LIB sync failed: " . $syncError->getMessage());
            }
        }
    }
    
    // Send notification to Budget Office if PPMP is marked as FINAL
    if ($isFinal) {
        try {
            // Get department name
            ...
            $notification->notifyBudgetAdmins($notificationType, $userId, $deptName, false);
            ...
            // Log error but don't fail the PPMP creation
    ...
    echo json_encode([
        'success' => true,
        'message' => ($ppmpType === 'supplemental' ? 'Supplemental PPMP' : 'PPMP') . ' created successfully' . ($libIdForRefresh ? '. Items synced to LIB.' : ''),
        'ppmp_id' => $ppmpId,
        'ppmp_type' => $ppmpType,
        'notification_sent' => $isFinal,
    ...
    error_log("Error in create_ppmp.php: " . $e->getMessage());
    echo json_encode([
        'success' => false,
