...
$userRole = isset($_SESSION['user_role']) ? $_SESSION['user_role'] : 'procurement';
...
// Calculate statistics - for procurement, we show all submissions from all departments
$allSubmissions = $fileSubmission->getAllSubmissions(100);
$totalSubmissions = count($allSubmissions);
$ppmpCount = count(array_filter($allSubmissions, function($s) { return $s['submission_type'] === 'PPMP'; }));
$libCount = count(array_filter($allSubmissions, function($s) { return $s['submission_type'] === 'LIB'; }));
$appCount = count(array_filter($allSubmissions, function($s) { return $s['submission_type'] === 'APP'; }));
$totalPending = count(array_filter($allSubmissions, function($s) { return $s['status'] === 'pending'; }));
$totalApproved = count(array_filter($allSubmissions, function($s) { return $s['status'] === 'approved'; }));

// Get latest submissions by type
    ...
    try {
        ...
        if ($totalBalance == 0) {
            ...
            if ($checkTable->rowCount() > 0) {
                ...
                if ($balanceData && $balanceData['total_balance'] !== null) {
...
<head>
    ...
    <title>BudgetTrack - Procurement Office</title>
        ...
        <div class="flex-1 flex flex-col" data-main-content>
                    ...
                    <div class="flex justify-between items-start">
                        <div class="text-white">
                            <div class="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    <h1 class="text-3xl font-bold mb-1">Procurement Office/BAC Dashboard</h1>
                                    <p class="text-red-100 text-sm">Welcome back, <span class="font-semibold"><?php echo htmlspecialchars($username); ?></span>! Manage procurement requests and track submissions.</p>
                                </div>
                            </div>
                            <div class="mt-3 flex gap-2">
                                ...
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-white bg-opacity-20 backdrop-blur-sm text-white border border-white border-opacity-30">
                                    Stats: <?php echo $totalSubmissions; ?> submissions • <?php echo $totalPending; ?> pending
                                </span>
                            </div>
                        </div>
            ...
            <div class="flex-1 p-4 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        try {
                            ...
                            if ($checkTable->rowCount() > 0) {
                                $budgetStmt = $conn->prepare("
                                    ...
                                    LIMIT 1
                                ");
                                $budgetStmt->execute([$departmentId, $fiscalYear]);
                    ...
                    ?>
                    <a href="allocations_view.php" class="bg-gradient-to-br from-maroon to-red-800 rounded-2xl shadow-lg border border-maroon p-8 hover:shadow-xl transition-all duration-300 text-white cursor-pointer block">
                        <div class="flex items-center justify-between mb-6">
                            <div class="flex items-center gap-4">
                                <div class="w-14 h-14 bg-white bg-opacity-20 rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                            ...
                            <button onclick="event.preventDefault(); showBudgetBreakdown(<?php echo $departmentId; ?>, <?php echo $fiscalYear; ?>);" class="px-4 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg text-sm font-semibold transition-colors border border-white border-opacity-30">
                                View Details
                            </button>
                            <?php endif; ?>
                        </div>
                        <p class="text-4xl font-bold text-white mb-2">₱<?php echo number_format($budgetAmount, 2); ?></p>
                        <p class="text-sm text-red-100">Fiscal Year <?php echo $fiscalYear; ?></p>
                    </a>

                    <!-- Total Balance for Utilization Card -->
                    <a href="utilization__view.php" class="bg-gradient-to-br <?php echo $totalBalance < 0 ? 'from-red-50 to-red-100 border-red-200' : 'from-green-50 to-green-100 border-green-200'; ?> rounded-2xl shadow-lg border p-8 hover:shadow-xl transition-all duration-300 cursor-pointer block">
                        <div class="flex items-center justify-between mb-6">
                            <div class="flex items-center gap-4">
                                <div class="w-14 h-14 bg-gradient-to-br <?php echo $totalBalance < 0 ? 'from-red-500 to-red-700' : 'from-green-500 to-green-700'; ?> rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                                    <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button onclick="event.preventDefault(); showUtilizationBreakdown(<?php echo $departmentId; ?>, <?php echo $fiscalYear; ?>);" class="px-4 py-2 <?php echo $totalBalance < 0 ? 'bg-red-200 hover:bg-red-300 text-red-800 border-red-300' : 'bg-green-200 hover:bg-green-300 text-green-800 border-green-300'; ?> rounded-lg text-sm font-semibold transition-colors border">
                                View Details
                            </button>
                            <?php endif; ?>
                        </div>
                        ...
                        <p class="text-sm text-gray-500">After Deductions</p>
                    </a>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <a href="file_submission.php" class="flex flex-col items-center justify-center px-6 py-5 bg-gradient-to-br from-maroon to-red-700 text-white rounded-xl hover:from-red-700 hover:to-red-800 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1">
                        ...
                        <a href="automated_reports.php" class="flex flex-col items-center justify-center px-6 py-5 bg-gradient-to-br from-indigo-500 to-indigo-600 text-white rounded-xl hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1">
                            <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                            </svg>
                            <span class="font-semibold text-center">Reports</span>
                        </a>
                        <a href="proc_notifications.php" class="flex flex-col items-center justify-center px-6 py-5 bg-gradient-to-br from-amber-500 to-amber-600 text-white rounded-xl hover:from-amber-600 hover:to-amber-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1 relative">
                            <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <a href="proc_notifications.php" class="text-sm text-maroon hover:text-maroon-dark font-medium">View all →</a>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-xl font-bold text-gray-900">Recent Submissions</h2>
                            <a href="track_requests.php" class="text-sm text-maroon hover:text-maroon-dark font-medium">View all →</a>
                        </div>
                        <div class="space-y-3">
                            <?php if (empty($submissions)): ?>
                                <div class="text-center py-12 text-gray-500">
                                    ...
                                    <p class="font-medium">No submissions</p>
                                    <p class="text-sm mt-1">Department submissions will appear here</p>
                                </div>
                            <?php else: ?>
                                <?php foreach (array_slice($submissions, 0, 5) as $submission): ?>
                                    <div class="flex items-center justify-between p-3 rounded-xl border-l-4 hover:bg-gray-50 transition-colors <?php 
                                        switch($submission['status']) {
                                            case 'approved': echo 'border-green-500 bg-green-50'; break;
                                            case 'rejected': echo 'border-red-500 bg-red-50'; break;
                                            case 'pending': echo 'border-yellow-500 bg-yellow-50'; break;
                                            default: echo 'border-blue-500 bg-blue-50'; break;
                                        }
                                    ?>">
                                        <div class="flex-1">
                                            <p class="font-semibold text-gray-900"><?php echo htmlspecialchars($submission['submission_type']); ?></p>
                                            <p class="text-xs text-gray-600 mt-1"><?php 
    ...
    <script>
        ...
        function showBudgetBreakdown(departmentId, fiscalYear) {
            ...
            fetch(`../api/get_budget_breakdown.php?department_id=${departmentId}&fiscal_year=${fiscalYear}`)
                ...
                .then(data => {
                    if (data.success) {
                        displayBudgetBreakdown(data.data);
                    } else {
                        ...
                        <table class="w-full border-collapse" style="min-width: 2000px;">
                            <thead>
                                <tr class="bg-maroon text-white">
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 250px;">General Description & Objective</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 120px;">Type</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 100px;">Qty</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 100px;">Unit</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 180px;">Recommended Mode</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-center" style="min-width: 80px;">Pre-Proc</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 120px;">Start</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 120px;">End Ads</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 120px;">Delivery</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 100px;">Source</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-right" style="min-width: 120px;">Budget</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-right" style="min-width: 120px;">Allocated</th>
                                    <th class="border border-gray-300 px-3 py-3 text-sm font-bold text-left" style="min-width: 150px;">Remarks</th>
                                </tr>
                            </thead>
                            <tbody id="ppmpItems-${ppmp.id}">
                                <tr><td colspan="13" class="text-center py-4 text-gray-500">Loading items...</td></tr>
                            </tbody>
                            <tfoot>
                                <tr class="bg-maroon text-white font-bold">
                                    <td colspan="10" class="border border-gray-300 px-3 py-3 text-right text-sm">GRAND TOTAL:</td>
                                    <td class="border border-gray-300 px-3 py-3 text-right text-sm" id="ppmpGrandTotal-${ppmp.id}">₱0.00</td>
                                    <td class="border border-gray-300 px-3 py-3 text-right text-sm" id="ppmpAllocatedTotal-${ppmp.id}">₱0.00</td>
        ...
        function populatePPMPItems(ppmpId, items) {
            ...
            if (items.length === 0) {
                tbody.innerHTML = '<tr><td colspan="13" class="text-center py-4 text-gray-500">No items found</td></tr>';
                return;
            }
            ...
            items.forEach(item => {
                ...
                html += `
                    <tr class="hover:bg-gray-50">
                        <td class="border border-gray-300 px-3 py-2 text-sm">${item.general_description || '-'}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${item.project_type || '-'}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm text-center">${item.quantity || '-'}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${item.unit || '-'}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${item.recommended_mode || '-'}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm text-center">${item.pre_procurement_conference || 'N'}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${formatDate(item.start_procurement)}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${formatDate(item.end_ads_posting)}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${formatDate(item.expected_delivery)}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${item.source_of_funds || '-'}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm text-right">₱${budget.toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm text-right">₱${allocated.toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
                        <td class="border border-gray-300 px-3 py-2 text-sm">${item.remarks || item.deducted_from_categories || '-'}</td>
                    </tr>
                `;
            });
            ...
            const allocatedTotalCell = document.getElementById(`ppmpAllocatedTotal-${ppmpId}`);
            if (grandTotalCell) grandTotalCell.textContent = '₱' + totalBudget.toLocaleString('en-US', {minimumFractionDigits: 2});
            if (allocatedTotalCell) allocatedTotalCell.textContent = '₱' + totalAllocated.toLocaleString('en-US', {minimumFractionDigits: 2});
